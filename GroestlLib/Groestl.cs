...
namespace GroestlLib
{
    public class Groestl
    {
        ...
        public void Execute()
        {
            ...
            t = (ulong)(length_input / bufferSize);
            if (length_input > 0)
            {
                for (int k = 0; k < (int)t; k++)
                {
                    byte[] charInByte = Encoding.Default.GetBytes(input.Substring(0, bufferSize));
                    for (int i = 0; i < bufferSize; i++)
                    {
                        buffer[i] = charInByte[i];
                    }
                    input = input.Remove(0, bufferSize);
                    h = Compression(h, buffer);
                }
            }
            byte[] charInByte2 = Encoding.Default.GetBytes(input);
            for (int i = 0; i < input.Length; i++)
            {
                buffer[i] = charInByte2[i];
            }
            int bytesRead = input.Length;
            t = (ulong)(length_input / bufferSize);
            N = N + (ulong)length_input * 8;
        ...
        public string GetHex(byte[] OutputData)
        {
            StringBuilder sb = new StringBuilder();
            foreach (var byt in OutputData)
            {
                string res = Convert.ToString(Convert.ToInt16(byt), 16);
                if (res.Length == 1)
                    res = "0" + res;
                sb.Append(res.ToUpper());
            }
            string result = sb.ToString();
            return result;
        }
        public string GetHex_Beautiful(byte[] OutputData)
        {
            StringBuilder sb = new StringBuilder();
            foreach (var byt in OutputData)
            {
